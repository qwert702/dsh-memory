window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = "" +
			...
			".dsh-mem-detailcard{flex:none;border:1px solid #4f8ef7;border-radius:10px;padding:10px;background:var(--dsw-alias-card-bg,transparent)}" +
			".dsh-mem-detailcard h4{margin:0 0 6px;font-size:12px;font-weight:600;color:var(--dsw-alias-label-secondary,#9a9a9a);display:flex;align-items:center;gap:8px}" +
			".dsh-mem-detailcard p{margin:0 0 6px;line-height:20px;white-space:pre-wrap;word-break:break-word}" +
			".dsh-mem-kv{display:flex;gap:6px;flex-wrap:wrap;font-size:11px;line-height:17px;color:var(--dsw-alias-label-secondary,#9a9a9a);margin-top:3px}" +
			".dsh-mem-linkjump{border:none;background:transparent;color:#4f8ef7;font-size:11px;line-height:17px;padding:0 4px;cursor:pointer}" +
			".dsh-mem-linkjump:hover{text-decoration:underline}";
		...
		const zh = {
			...
			created: "创建",
			updated: "更新",
			semOff: "语义召回不可用（可选依赖未装或模型下载失败），已回退关键词检索",
		};
		const en = {
			"view.memory": "Memory",
			...
			openSource: "Open source session",
			created: "created",
			updated: "updated",
			semOff: "Semantic recall unavailable (optional dep missing or model download failed); keyword fallback active",
		};
		//#endregion
		 ...
		 */
		const MemoryView = react.memo(function MemoryView({ t, sessionId: rawSessionId, openSession }) {
			...
			return react_jsx_runtime.jsxs("div", { className: "dsh-mem-root", "data-dsh-memory": true, children: [
				...
				react_jsx_runtime.jsxs("div", { className: "dsh-mem-status", children: [
					...
					statusInfo?.embeddings != null && statusInfo.embeddings.enabled === true && statusInfo.embeddings.state !== "ready" && statusInfo.embeddings.state !== "loading" && react_jsx_runtime.jsx("span", { className: "warn", children: "⚠ " + t("semOff") }, "sem"),
				...
				mode === "list" && selectedNode != null && (() => {
					const detail = items.find((item) => item.id === selectedNode);
					if (detail === undefined) return null;
					return react_jsx_runtime.jsxs("div", { className: "dsh-mem-detailcard", children: [
						react_jsx_runtime.jsxs("h4", { children: [
							react_jsx_runtime.jsx("span", { children: t("detailTitle") }),
							(detail.pinned === true || detail.status === "archived") && react_jsx_runtime.jsx("span", {}, "sp"),
							detail.pinned === true && react_jsx_runtime.jsx("span", { children: "📌" }, "pin"),
							detail.status === "archived" && react_jsx_runtime.jsx("span", { className: "dsh-mem-chip", children: t("archived") }, "arch"),
							react_jsx_runtime.jsx("button", { type: "button", className: "dsh-mem-act", style: { marginLeft: "auto" }, onClick: () => setSelectedNode(null), children: "✕" }, "close"),
						] }, "h"),
						react_jsx_runtime.jsx("p", { children: detail.content }, "p"),
						react_jsx_runtime.jsxs("div", { className: "dsh-mem-kv", children: [
							react_jsx_runtime.jsx("span", { className: "dsh-mem-typechip", style: { background: TYPE_COLORS[detail.type] ?? "#888" }, children: t("type." + detail.type) }, "type"),
							...detail.tags.map((tag) => react_jsx_runtime.jsx("span", { className: "dsh-mem-chip", children: tag }, "tag:" + tag)),
							react_jsx_runtime.jsx("span", { children: (ORIGIN_ICON[detail.origin] ?? "•") + " " + t("created") + " " + timeAgo(detail.createdAt) }, "created"),
							react_jsx_runtime.jsx("span", { children: t("updated") + " " + timeAgo(detail.updatedAt) }, "updated"),
							detail.useCount > 0 && react_jsx_runtime.jsx("span", { children: "×" + detail.useCount }, "uses"),
						] }, "kv"),
						detail.links.length > 0 && react_jsx_runtime.jsxs("div", { className: "dsh-mem-kv", children: [
							react_jsx_runtime.jsx("span", { children: "⧉" }, "lk"),
							detail.links.map((edge) => {
								const target = items.find((it) => it.id === edge.id);
								if (target === undefined) return null;
								return react_jsx_runtime.jsx("button", {
									type: "button",
									className: "dsh-mem-linkjump",
									onClick: () => setSelectedNode(edge.id),
									children: "[" + edge.kind + "] " + (target.content.length > 18 ? target.content.slice(0, 18) + "…" : target.content),
								}, edge.id);
							}),
						] }, "links"),
						typeof detail.sourceSessionId === "string" && detail.sourceSessionId !== "" && openSession != null && react_jsx_runtime.jsx("div", { className: "dsh-mem-kv", children:
							react_jsx_runtime.jsx("button", { type: "button", className: "dsh-mem-linkjump", onClick: () => openSession(detail.sourceSessionId), children: "↗ " + t("openSource") }, "src")
						}, "source"),
					] }, "detailcard");
				})(),
		...
		function apply(ctx) {
			...
			const BoundView = (props) => react_jsx_runtime.jsx(MemoryView, { ...props, t, openSession: (id) => { try { ctx.sessions.open(id); } catch {} } });

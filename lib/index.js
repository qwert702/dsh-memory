...
export function apply(ctx) {
  ...
  // Same bounded-lifetime rule for the driver classification table: sessions
  // vanish from it (and re-classify on their next user message) once the cap
  // is hit, so a months-long harness cannot grow it without bound.
  ...
  ctx.on('system-prompt/assemble', async (assembly, context, next) => {
    ...
    try {
      ...
      if (queryVector !== undefined) {
        // Merge both scopes' vector maps (ids are unique across stores).
        itemVectors = new Map([...(await runtime.vectors.load('global')), ...(cwd === undefined ? [] : await runtime.vectors.load(projectKeyFor(cwd)))])
      }
    ...
    try {
      ...
      if (event.type === 'user/message') {
        ...
        if (isMachine && runtime.sessionKinds.get(session.id) === undefined) {
          // Bounded growth: drop the oldest classification when over cap.
          if (runtime.sessionKinds.size >= SESSION_KINDS_MAX) {
            const oldest = runtime.sessionKinds.keys().next().value
            if (oldest !== undefined) runtime.sessionKinds.delete(oldest)
          }
          runtime.sessionKinds.set(session.id, 'machine')
        }
        ...
        if (!isMachine) runtime.sessionKinds.set(session.id, 'human')
      ...
      handler: async (req, res) => {
        try {
          ...
          send(res, 200, {
            ...
            embeddings: { enabled: config.embeddingsEnabled === true, state: runtime.embedder.state, lastError: runtime.embedder.lastError },

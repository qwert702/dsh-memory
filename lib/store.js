...
export class MemoryStore {
   ...
   *
   * Error policy: ENOENT is a normal first run and yields the empty shell.
   * ANY other failure (corrupt JSON, missing items array, unreadable file)
   * must never masquerade as an empty store — a mutation built on that lie
   * would atomically overwrite the real file and wipe the library. Instead
   * the suspect file is renamed aside as `<name>.corrupt-<timestamp>` (best
   * effort, keeping every byte for manual recovery) and the error is
   * rethrown so the mutation aborts: pausing extraction beats losing data.
   * @param storeKey - the scope to load.
   * @returns the parsed file shell (always has `items`; links/tags coerced).
   */
  async readFresh(storeKey) {
    ...
    try {
      ...
      if (error !== null && typeof error === 'object' && error.code === 'ENOENT') return data
    ...
    try {
      ...
      await this.#quarantine(file)
      throw coded('store-corrupt', `${path.basename(file)} is not valid JSON (${error instanceof Error ? error.message : String(error)})`)
    }
    if (parsed === null || typeof parsed !== 'object' || !Array.isArray(parsed.items)) {
      await this.#quarantine(file)
      throw coded('store-corrupt', `${path.basename(file)} has no items array`)
    }
    data.cwd = typeof parsed.cwd === 'string' ? parsed.cwd : undefined
    // Defensive coercion at the single read chokepoint: entries missing
    // links/tags (hand edits, partial imports) get empty arrays so every
    // downstream consumer (graph projection, export, briefing scoring,
    // link mutations) sees well-formed items, and the next save persists
    // the repaired shape.
  ...
  /**
   * Move a suspect store file out of harm's way before any further write can
   * touch it (`<name>.corrupt-<timestamp>`). Best effort: if even the rename
   * fails (e.g. a brief AV lock), the original read error still aborts the
   * caller and the next attempt retries the quarantine.
   * @param file - the store file to set aside.
   */
 ...
 * Consolidation is special: instead of folding relative increments it writes
 * an ABSOLUTE 0 for the scope (idempotent under retry/replay), and any
 * increments captured in the same batch apply strictly AFTER that zero, so
 * a slow flush racing markConsolidated cannot resurrect the old count.
 */
export class PluginState {
  #data = { version: 1, cursors: {}, turnCounts: {}, lastConsolidation: {} }
  /**
   * Uncommitted deltas: cursors {id:seq}, turnIncr {key:n} (relative +1s),
   * turnReset Set<key> (absolute zero written by consolidation, merged BEFORE
   * any turnIncr of the same batch), consolidated {key:record}.
   */
  #pending = { cursors: new Map(), turnIncr: new Map(), turnReset: new Set(), consolidated: new Map() }
  #timer
  #flushing = false
   ...
   * double-cleared. Turn counters merge RESETS first (absolute 0 written
   * verbatim, so retries and replays stay idempotent) and only then the
   * relative increments. Failures put the batch back and retry on next
   * schedule.
   */
  async flush() {
    if (this.#flushing) return
    if (this.#pending.cursors.size === 0 && this.#pending.turnIncr.size === 0 && this.#pending.turnReset.size === 0 && this.#pending.consolidated.size === 0) return
    ...
    try {
      await withFileLock(file, async () => {
        ...
        // Reset BEFORE increments: the absolute zero wins over any increment
        // snapshotted before markConsolidated ran, while later bumps (fresh
        // pending increments) still apply on top. Writing the literal 0 keeps
        // the merge idempotent across processes and retry storms.
        for (const key of batch.turnReset) disk.turnCounts[key] = 0
        ...
        // turnCounts intentionally NOT mirrored here: the getter folds the
        // pending increments onto the local base, and markConsolidated has
        // already zeroed that base and queued the matching reset above.
      ...
      for (const key of batch.turnReset) this.#pending.turnReset.add(key)
   ...
   * merge is reset-aware — consolidation writes an absolute 0 first and any
   * increments apply after it, so hosts converge on 0 instead of drifting).
   * @param storeKey - the scope.
  ...
  markConsolidated(storeKey, applied) {
    // Drop uncommitted increments AND queue an absolute zero so the next
    // flush writes 0 to disk instead of folding the stale increments: an
    // older batch already in flight can no longer resurrect the previous
    // count after consolidation — every process converges on 0.
    this.#pending.turnIncr.delete(storeKey)
    this.#pending.turnReset.add(storeKey)

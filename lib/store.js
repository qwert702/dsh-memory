      ...
      if (action !== 'add' && typeof targetId === 'string') {
        ...
        if (target !== undefined) {
          ...
          if (action === 'supersede') {
            ...
            item.links.push({ id: target.id, kind: 'supersedes' })
          }
          items.push(item)
          await this.saveStore(storeKey, data)
...
export class PluginState {
   ...
   * across processes). Failures keep the deltas and retry on next schedule.
   */
  async flush() {
    ...
    try {
      await withFileLock(file, async () => {
        ...
        for (const [id, seq] of this.#pending.cursors) disk.cursors[id] = Math.max(disk.cursors[id] ?? 0, seq)
        for (const [key, incr] of this.#pending.turnIncr) disk.turnCounts[key] = (disk.turnCounts[key] ?? 0) + incr
        for (const [key, record] of this.#pending.consolidated) disk.lastConsolidation[key] = record
        await mkdir(path.dirname(file), { recursive: true })
        ...
        for (const [id, seq] of this.#pending.cursors) this.#data.cursors[id] = Math.max(this.#data.cursors[id] ?? 0, seq)
        for (const [key, incr] of this.#pending.turnIncr) this.#data.turnCounts[key] = (this.#data.turnCounts[key] ?? 0) + incr
        for (const [key, record] of this.#pending.consolidated) this.#data.lastConsolidation[key] = record

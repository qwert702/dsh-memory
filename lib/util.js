 ...
 */
function relevanceOf(item, queryTokens) {
 ...
 * rest ranked by `importance x (1 + relevanceWeight x relevance)` with an
 * interleaved global/project sequence so neither scope crowds the other out.
 * When `queryTokens` is empty the ranking degenerates to pure importance —
 ...
 * @param queryTokens - optional recent-conversation token set for relevance.
 * @returns the briefing text ('' when nothing to say), picked ids, and how
 *   many were pinned.
 */
export function selectBriefing(globalItems, projectItems, topK, maxChars, queryTokens) {
  ...
  const rank = (list) => [...list].sort((a, b) => {
    const sa = importanceScore(a) * (1 + 10 * relevanceOf(a, queryTokens))
    const sb = importanceScore(b) * (1 + 10 * relevanceOf(b, queryTokens))
    return sb - sa
  })

 ...
 * trailing ./-/_ stripped so `rust.` matches `rust`) plus CJK character
 * bigrams, lowercased. Returns a Set for fast overlap.
...
export function tokenize(text) {
  ...
  for (const word of s.matchAll(/[a-z0-9][a-z0-9._-]+/g)) {
    const stem = word[0].replace(/[./_-]+$/, '')
...
/** Recency decay over days: the 21-day figure is the e-folding time constant of Math.exp(-days/21); the corresponding half-life is ~14.6 days. */
...
/** The wrapper markers emitted by selectBriefing below. Untrusted memory
 * content is scanned for these exact sequences and destructively neutralized
 * before line assembly (review P1-5 prompt-injection hardening). */
const BRIEFING_MARKERS = ['<memory-briefing>', '</memory-briefing>']

/**
 * Neutralize briefing wrapper markers inside untrusted content: a zero-width
 * space (\u200B) is inserted before EVERY angle bracket ('<' and '>') of each
 * marker occurrence, breaking up the exact sequences so memory text can
 * never open or close the injected briefing block. Visually invisible;
 * budget accounting still measures the final assembled line.
 * @param text - raw memory content.
 * @returns the defanged text.
 */
function sanitizeBriefingContent(text) {
  let s = String(text)
  for (const marker of BRIEFING_MARKERS) {
    const defanged = marker.replace(/([<>])/g, '\u200B$1')
    s = s.split(marker).join(defanged)
  }
  return s
}

/**
 * Origin label rendered at line start (review P1-5): extracted material
 * (auto extraction and consolidation merges) is marked as reference data
 * rather than instructions; manual entries say so too.
 * @param origin - item origin ('auto' | 'manual' | 'consolidation').
 * @returns the prefix ('' for unknown origins).
 */
function originMark(origin) {
  if (origin === 'auto' || origin === 'consolidation') return '〔对话提取资料〕'
...
export function selectBriefing(globalItems, projectItems, topK, maxChars, queryTokens, queryVector, itemVectors) {
  ...
  for (const item of [...pinned.slice(0, topK), ...picked]) {
    ...
    const line = prefix + pinMark + originMark(item.origin) + sanitizeBriefingContent(item.content)
